...
module FFakerTW
  module PhoneNumberDE
    ...
    def region_prefix(leading_zero = true)
      region_prefix = rand(2..9).to_s + FFakerTW.numerify('#' * rand(1..3)).to_s
    ...
    def mobile_phone_number(leading_zero = true)
      [mobile_prefix(leading_zero), FFakerTW.numerify('#######')].join(' ')
    end

    def home_work_phone_number(leading_zero = true)
      [region_prefix(leading_zero), FFakerTW.numerify('#######')].join(' ')
    end
