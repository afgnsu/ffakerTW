...
module FFakerTW
  module Movie
    ...
    def title_from_formula
      ...
      when 3 then "#{simple_title}: The #{FFakerTW::Name.name} Story"
      when 4 then "When #{FFakerTW::Name.first_name} Met #{FFakerTW::Name.first_name}"
      when 5 then "Dr. #{fetch_sample(NOUNS)}"
      when 6 then "Je Vous Presente, #{FFakerTW::Name.first_name}"
      when 7 then "#{rand(0...2999)} A.D."
      when 8 then "The #{fetch_sample(NOUNS)} from #{FFakerTW::Address.neighborhood}"
      when 9 then "Christmas on #{FFakerTW::Address.street_name}"
      when 10 then "The #{fetch_sample(COLORS)} Rose of #{FFakerTW::AddressUK.country}"
