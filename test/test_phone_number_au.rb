...
class TestPhoneNumberAU < Test::Unit::TestCase
  ...
  def setup
    @tester = FFakerTW::PhoneNumberAU
  end

  def test_mobile_phone_prefix
    assert_include(FFakerTW::PhoneNumberAU::MOBILE_OPERATORS_PREFIX,
                   @tester.mobile_phone_prefix)
  end

  def test_home_work_phone_prefix
    assert_include(FFakerTW::PhoneNumberAU::HOME_WORK_OPERATORS_PREFIX,
                   @tester.home_work_phone_prefix)
  end

  def test_phone_prefix
    assert_include(FFakerTW::PhoneNumberAU::OPERATORS_PREFIX,
                   @tester.phone_prefix)
  end

  def test_mobile_phone_number
    assert_match(/0[45]\d{2} \d{3} \d{3}/,
                 FFakerTW::PhoneNumberAU.mobile_phone_number)
  end

  def test_home_work_phone_number
    assert_match(/\(\d{2}\) \d{4} \d{4}/,
                 FFakerTW::PhoneNumberAU.home_work_phone_number)
  end

  def test_phone_number
    assert_match(/(?:0[45]\d{1,2} \d{3} \d{3}|\(\d{2}\) \d{4} \d{4})/,
                 FFakerTW::PhoneNumberAU.phone_number)
  end

  def test_international_mobile_phone_number
    assert_match(/\+61 \d{1,2} \d{4} \d{4}/,
                 FFakerTW::PhoneNumberAU.international_mobile_phone_number)
  end

  def test_international_home_work_phone_number
    assert_match(/\+61 \d{1,2} \d{4} \d{4}/,
                 FFakerTW::PhoneNumberAU.international_home_work_phone_number)
  end

  def test_international_phone_number
    assert_match(/\+61 \d{1,2} \d{4} \d{4}/,
                 FFakerTW::PhoneNumberAU.international_phone_number)
  end
end
