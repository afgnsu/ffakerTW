...
class TestGeolocation < Test::Unit::TestCase
  ...
  assert_methods_are_deterministic(FFakerTW::Geolocation, :lat, :lng)

  def test_lat
    assert_match(/[0-9]+/, FFakerTW::Geolocation.lat.to_s)
  end

  def test_lng
    assert_match(/[0-9]+/, FFakerTW::Geolocation.lng.to_s)
  end
  ...
  def test_boxed_coords
    ...
    coords_list.each do |coords|
      ...
      new_coords = FFakerTW::Geolocation.boxed_coords(coords[0], coords[1])
