...
class TestPhoneNumberCU < Test::Unit::TestCase
  ...
  def setup
    @tester = FFakerTW::PhoneNumberCU
  end

  def test_mobile_phone_prefix
    assert FFakerTW::PhoneNumberCU::MOBILE_OPERATORS_PREFIX.include?(
      @tester.mobile_phone_prefix
    )
  end

  def test_home_work_phone_prefix
    assert FFakerTW::PhoneNumberCU::HOME_WORK_OPERATORS_PREFIX.include?(
  ...
  def test_phone_prefix
    10.times do
      assert FFakerTW::PhoneNumberCU::OPERATORS_PREFIX.include?(
        @tester.phone_prefix
      )
    end
  end
  ...
  def test_e164_mobile_phone_number
    assert_match(/535\d{7}/,
                 FFakerTW::PhoneNumberCU.e164_mobile_phone_number)
  end

  def test_e164_home_work_phone_number
    assert_match(/53(7\d{7}|\d{2}\d{6})/,
                 FFakerTW::PhoneNumberCU.e164_home_work_phone_number)
  end

  def test_e164_phone_number
    10.times do
      assert_match(/53((5|7)\d{7}|\d{2}\d{6})/,
                   FFakerTW::PhoneNumberCU.e164_phone_number)
    end
  end
end
