...
class TestFakerNameJA < Test::Unit::TestCase
  class << self
    def startup
      FFakerTW::NameJA.const_set(:FIRST_NAMES, %w[あきら フミ 三郎])
      FFakerTW::NameJA.const_set(:LAST_NAMES, %w[佐藤 高橋 佐々木])
    end

    def shutdown
      FFakerTW::NameJA.class_eval { remove_const(:FIRST_NAMES) }
      FFakerTW::NameJA.class_eval { remove_const(:LAST_NAMES) }
    end
  end
