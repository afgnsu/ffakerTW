...
class TestArrayUtils < Test::Unit::TestCase
  ...
  def setup
    @array = FFakerTW::ArrayUtils.const_array('a'..'c')
    @elems = Set.new('a'..'c')
  end
  ...
  def test_self_random_pick_is_deterministic
    supress_warn_output do
      ...
      assert_deterministic { FFakerTW::ArrayUtils.random_pick(array, 3) }
    end
  end
  ...
  def test_self_rand_is_deterministic
    supress_warn_output do
      array = Array.new(10) { rand(0..9) }
      assert_deterministic { FFakerTW::ArrayUtils.rand(array) }
    end
  end

  def test_self_shuffle_is_deterministic
    array = Array.new(10) { rand(0..9) }
    assert_deterministic { FFakerTW::ArrayUtils.shuffle(array) }
  end
